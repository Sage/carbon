...
- **npm**: `>=11.14.1` (CI runs `npm install -g npm@11.14.1` before every job — do the same if the local npm is older).
...
- Prefer `data-role` for test queries (RTL and Playwright are both configured to read `data-role` as their test-id attribute). `data-component` is reserved for component roots, `data-element` for sub-elements. Don't introduce new `data-testid` attributes in component source or `*.spec.*`/`*.pw.*` tests; the only accepted use is inside Storybook interaction stories (e.g. `*-interaction.stories.tsx`, `*-test.stories.tsx`) where the Storybook test runner / play functions expect plain `data-testid` selectors.
...
7. **Skip review on generated / low-signal paths.** Do not leave review comments on files in the following paths — they are either generated, tracked for tooling reasons, or lack the source-level context that makes a review comment useful. Trust the corresponding source change and CI checks instead:
   - `skills/**` — generated/maintained by `npm run build:skills`; CI enforces freshness via `npm run build:skills -- --check`. Review the underlying component change, not the regenerated skill file.
