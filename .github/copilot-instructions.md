...
- Public API is defined by exports from component `index.ts` files and `src/index.ts`, not source location. Unexported code, including code under `__internal__/`, is private. Components live under `src/components/`; hooks, locales, and shared styles live under `src/hooks/`, `src/locales/`, and `src/style/`.
...
- Use Conventional Comments for every review comment: `<label> [decorations]: <subject>`. Use labels `issue:`, `suggestion:`, `question:`, `nitpick:`, `praise:`, `thought:`, `chore:`, or `note:`. Decorations are optional. Use `(blocking)` only when the finding must be resolved before merge, `(non-blocking)` when it is useful to clarify that feedback may be deferred, and `(if-minor)` when resolution depends on the PR's scope. Otherwise, omit the decoration.

### Carbon review checks

- **Public API and deprecations:** Adding a deprecation notice is non-breaking. Removing or renaming a public API remains breaking even when it was previously deprecated, as do newly required props, narrowed types, removed union members, and changed defaults or behavior. Require a `BREAKING CHANGE:` commit footer with clear migration guidance for those breaking changes. Ignore APIs not exported from supported public entry points.
...
- **Configuration drift:** Check whether instruction updates are needed when changing Node/npm versions, major dependencies, npm scripts, lint or coverage settings, test configuration, root config names, commit conventions, or component layout conventions.
