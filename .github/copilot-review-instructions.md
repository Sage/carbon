...
- Remove/rename export from component `index.ts` or `src/index.ts`
- Remove/rename `Props` interface or required props
- Narrow prop type, remove union members, change default affecting output
- Remove `data-component`/`data-element`/`data-role` value or stable CSS class
- Remove Storybook story documenting supported usage
