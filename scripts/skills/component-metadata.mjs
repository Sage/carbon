...
export function validateComponentMetadata(
  ...
  requireTextArray(value.useWhen, filePath, "useWhen", true);
  ...
  if (value.alternatives !== undefined) {
    if (!Array.isArray(value.alternatives)) {
      fail(filePath, 'field "alternatives" must be an array');
    }
  ...
  if (value.examples !== undefined) {
    if (!Array.isArray(value.examples)) {
      fail(filePath, 'field "examples" must be an array');
    }
...
function requireTextArray(value, filePath, field, required = false) {
  ...
  if (!Array.isArray(value) || (required && value.length === 0)) {
    fail(
      filePath,
      `field \"${field}\" must be ${required ? "a non-empty" : "an"} array`,
    );
  }
