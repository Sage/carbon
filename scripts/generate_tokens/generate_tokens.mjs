...
// For any tokens that need to be imported into component code, we generate separate files for them.
// If there is a __next__ directory, we put them in there, if not we put them in __internal__ 
// since they are only for internal use and not part of the public API
const componentTokenConfigs = [
  {
    component: "dialog",
    targetDir: "src/components/dialog/__internal__/__next__/__internal__/tokens",
    imports: [
      {
        module: "@sage/design-tokens-fusion/js/es6/components/container.js",
        names: [
...
const generateComponentTokenFiles = async (configs) => {
  for (const config of configs) {
    ...
    for (const { module: mod, names } of config.imports) {
    ...
    const dir = path.join(projectRoot, config.targetDir);
...
await generateComponentTokenFiles(componentTokenConfigs);
