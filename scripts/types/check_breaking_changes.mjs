...
if (breaking.length === 0 || hasBreakingMarker(commitMsg)) {
  process.exit(0);
}
...
/**
 * Detects Conventional Commit breaking markers in a commit message.
 * Supports:
 *  - footer token: BREAKING CHANGE: ...
 *  - footer token: BREAKING-CHANGE: ...
 *  - type/scope bang: feat!: ... / feat(scope)!: ...
 *
 * Matching is case-insensitive to avoid false negatives from editor templates.
 *
 * @param {string} message
 * @returns {boolean}
 */
function hasBreakingMarker(message) {
  const hasBreakingFooter = /^BREAKING[ -]CHANGE\b/im.test(message);
  const hasBangHeader = /^[a-z]+(?:\([^)]+\))?!:/im.test(message);
  return hasBreakingFooter || hasBangHeader;
}
