...
describe('withUniqueName HOC', () => {
  ...
  it('adds a name prop to a component passed as a paramater', () => {
    ...
    expect(wrapper.find(InputComponent).first().childAt(0).childAt(0).props().name).not.toEqual(undefined);
  });
  ...
  it('the generated name prop is persisted even after re-render', () => {
    ...
    expect(wrapper2.find(InputComponent).first().childAt(0).childAt(0).props().name).not.toEqual(wrapper.props().name);
