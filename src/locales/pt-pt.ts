...
const ptPT: Partial<Locale> = {
  ...
  tabs: {
    error: (tabTitle) => `O separador ${tabTitle} contém erros`,
    info: (tabTitle) => `O separador ${tabTitle} contém informações`,
    warning: (tabTitle) => `O separador ${tabTitle} contém avisos`,
  },
  textEditor: {
    boldAria: () => "Negrito",
    ...
    hyperlink: {
      ...
      formKey: () => "Indica um campo obrigatório",
      linkFieldLabel: () => "Endereço do link",
      linkFieldErrorMessage: () => "Introduza o endereço do link",
      saveButton: () => "Guardar",
      saveButtonAria: () => "Guardar",
      textFieldLabel: () => "Texto do link",
      textFieldErrorMessage: () => "Introduza o texto do link",
    },
    typography: {
  ...
  search: {
    ...
    assistiveHint: () =>
      "Quando existirem sugestões de pesquisa, utilize as teclas de seta para cima e para baixo para percorrer os resultados e prima Enter para selecionar. Nos dispositivos táteis, explore os resultados através do toque ou de gestos de deslize.",
    queryTooShort: (minQueryLength) =>
      `Introduza pelo menos ${minQueryLength} carateres para obter resultados.`,
    noResults: () => "Não foram encontrados resultados",
    results: (length) => {
      const words = {
        result: length === 1 ? "resultado" : "resultados",
        available: length === 1 ? "disponível" : "disponíveis",
      };
