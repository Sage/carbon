import React, { useContext, useEffect, useRef, useImperativeHandle, forwardRef, useState, useCallback, useMemo, CSSProperties } from "react";
...
const DraggableContainer = forwardRef<UseDraggableHandle, DraggableContainerProps>(({
  ...
  const localRegister = (items: React.ReactNode | React.ReactNode[]) => {
    setList((prevList) => Array.isArray(items) ? [...prevList, ...items] : [...prevList, items]);
  };

const localMove = useCallback((itemId: string | number, toIndex: number) => {
    // currently relies on the children of draggable item having a unique id 
    const fromIndex = list.findIndex((item) => React.isValidElement(item) && item.props.children.props.id === itemId);    
  ...
  useEffect(() => {
    ...
    monitorForElements({
      ...
      onDrop({ location, source }) {
        ...
        // if move is not passed via context, use local move func instead
        if(!move){localMove(destinationId, indexOfTarget)}        
      },
    })
  )
  }, [effectiveList, id, localMove, move]);

  return (
    <DraggableContainerContext.Provider value={{ columnId: id || uniqueId.current }}>
      {React.createElement(
        containerNode,
        {
