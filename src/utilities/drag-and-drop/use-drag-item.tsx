...
interface UseDragItemProps {
  ...
  list: string;
  /** Optional type describing the item. */
  type?: string;
}
...
function useDragItem({
  ...
  list,
  type,
}: UseDragItemProps): UseDragItemReturn {
  ...
  useEffect(() => {
    ...
    return combine(
      draggable({
        element,
        getInitialData: () => getItem({ id, instanceId, index, list, type }),
      }),
      dropTargetForElements({
        element,
        getData: () => getItem({ id, instanceId, index, list, type }),
        canDrop: ({ source }) =>
          isItem(source.data) && source.data.instanceId === instanceId,
      }),
    );
  }, [id, index, list, instanceId, type]);
