...
const StyledColorOptions = styled.div`
  max-width: ${({ maxWidth, childWidth }) => getRoundedMaxWidth(maxWidth, childWidth)}px;
  ...
  ${({
    ...
    if (error) return css`outline: 2px solid ${theme.colors.error}`;
    if (warning) return css`outline: 1px solid ${theme.colors.warning}`;
    if (info) return css`outline: 1px solid ${theme.colors.info}`;
    return '';
  }}
`;
