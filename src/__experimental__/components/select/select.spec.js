...
const multiValueReturn = options.map(({ value, text }) => ({
  optionValue: value,
  optionText: text,
}));
...
describe("Select", () => {
    ...
    type(
      <Select {...props}>
        {selectOptions.map(({ value, text }) => (
          <Option key={text} text={text} value={value} />
        ))}
      </Select>
    );
  ...
  it("applies custom data-* attributes at the right level", () => {
    ...
    expect(wrapper.find("Textbox").first().prop("data-role")).toEqual(
      "custom-role"
    );
    expect(wrapper.find("Textbox").first().prop("data-element")).toEqual(
      "custom-element"
    );
  });
  ...
  describe.each([
    ...
    it(`renders the correct placeholder when disabled is ${props.disabled} and readOnly is ${props.readOnly}`, () => {
      ...
      expect(wrapper.find(Input).props().placeholder).toEqual(
        canRenderPlaceholder ? "Please Select..." : ""
      );
    });
  });
    ...
    it("triggers onChange with the item added when choosing an item", () => {
      ...
      expect(props.onChange).toHaveBeenCalledWith({
        target: {
          ...
          value: [
            ...multiValueReturn,
            { optionText: "New Value!", optionValue: "new" },
          ],
        },
      });
    });
  ...
  describe("when backspace is pressed and is single select", () => {
    ...
    it("does not trigger onChange if there is a filter in effect", () => {
      const { props } = setupTest((wrapper) =>
        wrapper.setState({ filter: "x" })
      );
      expect(props.onChange).not.toHaveBeenCalled();
    });
  ...
  describe("when single value", () => {
    ...
    it("triggers onChange with the new item when choosing an item", () => {
      ...
      expect(props.onChange).toHaveBeenCalledWith({
        target: {
          value: [{ optionText: newValue.text, optionValue: newValue.value }],
        },
      });
    });
  ...
  describe("when in classic theme", () => {
    it("it applies expected icon styling", () => {
      const styleWrapper = TestRenderer.create(
        <StyledSelect theme={classic}>
          <StyledIcon type="dropdown" />
        </StyledSelect>
      );
      assertStyleMatch({ color: "#FFFFFF" }, styleWrapper.toJSON(), {
  ...
  describe("renders select with one option", () => {
    it("renders correctly", () => {
      const wrapper = mount(
        <Select value="1">
          <Option key="Orange" text="Orange" value="1" />
        </Select>
      );
      wrapper.find("input").simulate("focus");
    ...
    it("triggers onChange with one item when choosing the item", () => {
      ...
      expect(props.onChange).toHaveBeenCalledWith({
        target: {
          value: [
            { optionText: options[0].text, optionValue: options[0].value },
          ],
        },
      });
    });
  ...
  describe("external validations", () => {
    it.each([["error"], ["warning"], ["info"]])(
      "should pass %s validation prop to the Textbox component",
      (validation) => {
  ...
  it("warns about deprecation", () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      "[Deprecation] The `lib/__experimental__/select` component is deprecated and will soon be removed. Please use `lib/select` instead."
    );
  });
});
