...
const groupCheckboxKeys = Object.keys(groupCheckboxes);
...
const groupCheckbox = groupCheckboxKeys.filter((name) => {
  ...
  <div>
    ...
    >
      {groupCheckbox.map(id => (
        <State store={ groupCheckboxes[id].store } key={ `check-state-${id}` }>
          <Checkbox
