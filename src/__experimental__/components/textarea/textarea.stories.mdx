...
<Meta title="Experimental/Textarea" parameters={{ info: { disable: true } }} />
...
<Preview>
  <Story name="with characterLimit - not enforced" parameters={{ chromatic: { disable: true } }}>
    {() => {
      const [value, setValue] = useState("");
      return (
        <Textarea
...
<Preview>
  <Story name="with labelHelp">
    <Textarea label="Textarea" labelHelp="Help" />
  </Story>
</Preview>
