...
* Error or warning message that interrupts activity? [Try Alert](/components/alert).
 `,
  ...
  propDescriptions: {
    as:
      "Sets the theme of the notification. Possible values include: " +
      OptionsHelper.colors.join(", "),
    ...
    timeout:
      "Sets the time in Milliseconds the flash remains on the screen. After the timeout it will call the onDimiss callback. This will remove the close icon when set",
  },
  openPreview: true,
});
