...
describe("Flash", () => {
  ...
  beforeEach(() => {
    ...
    defaultInstance = TestUtils.renderIntoDocument(
      <Flash open onDismiss={dismissHandler} message="Danger Will Robinson!" />
    );
  ...
  describe("props", () => {
    it("should pass id prop to the root component", () => {
      const wrapper = shallow(<Flash open id="flash-id" />);
  ...
  describe("constructor", () => {
    ...
    describe("when this.props.open is true", () => {
      it("sets this.state.open to true", () => {
        const wrapper = shallow(<Flash open {...commonProps} />);
    ...
    describe("when this.props.open is false", () => {
      it("sets this.state.open to false", () => {
        const wrapper = shallow(<Flash open={false} {...commonProps} />);
  ...
  describe("formatDescription", () => {
    ...
    describe("if the item is an object", () => {
      it("adds each item", () => {
        const items = defaultInstance.formatDescription({ foo: "bar" });
        expect(
          items.props.children[0].props.children.props.children[0]
        ).toEqual("foo");
        expect(
          items.props.children[0].props.children.props.children[1]
        ).toEqual(": ");
        expect(
          items.props.children[0].props.children.props.children[2]
        ).toEqual("bar");
      });
    });
      ...
      describe.each(iconVariant)(
        ...
        (option, variant) => {
          it(`should render ${variant} variant of the Flash component`, () => {
            ...
            expect(
              flashInfo.find(FlashIconStyle).props({ type: variant })
            ).toBeTruthy();
          });
        }
      );
  ...
  describe("render", () => {
    ...
    describe("when the flash is open", () => {
      it("renders an outer slider element", () => {
        const outerSlider = flashInstance.find(
          'div[data-element="flash-slider"]'
        );
