...
const StyledTitleContent = styled.div`
  ...
  ${({
    ...
    ${size === 'large' && position === 'top' && css`
      padding: 10px 24px;
      font-size: 16px;
      ${isTabSelected && !hasSiblings && !(error || warning || info) && css`padding-bottom: 6px;`}
    `}

    ${size === 'large' && position === 'left' && css`
      font-size: 16px;
      padding: 22px 24px;
    `}
  ...
  ${({
    ...
    size,
    isTabSelected
  }) => hasSiblings && !hasCustomLayout && css`
    ${size === 'default' && css`
      padding-top: 11px 
      padding-bottom: 8px;

      ${!(error || warning || info) && isTabSelected && css`
        padding-bottom: 6px;
      `}
    `}
...
const StyledTabTitle = styled.li`
  ...
  ${({
    ...
    border-bottom: 2px solid ${alternateStyling ? `${theme.tab.background}` : `${theme.colors.primary}`};
    ${size === 'large' && css`
  ...
  ${({
    ...
    ${!alternateStyling && css`border-right: 2px solid ${theme.tab.background};`}
    display: block;
    height: auto;
    margin-left: 0px;
    ...
    &:hover {
      ...
      `}
      border-right-color: ${alternateStyling ? `${theme.tab.background};` : ''}
      ${error || warning || info ? 'border-right: none;' : ''}
    }

    ${({ isTabSelected }) => isTabSelected && css`
      border-right-color: ${alternateStyling ? `${theme.tab.background};` : `${theme.colors.primary};`}
      background-color: ${theme.colors.white};
      ${error || warning || info ? 'border-right: none;' : ''}
...
const StyledLayoutWrapper = styled.div`
  ${({ hasCustomLayout }) => hasCustomLayout && css`
    flex-grow: 2;
    min-width: 100px;
  `}

  ${({ hasCustomLayout, titlePosition, hasCustomSibling }) => !hasCustomLayout && css`
    display: inline-flex;
    
    ${({ theme }) => css`
      position: relative;
      top: -1px;
      ${hasCustomSibling && css`
        left: 4px;
      `}
      ${!hasCustomSibling && css`
