...
describe('TabHeader', () => {
  ...
  describe('when onKeyDown', () => {
    it('triggers onKeyDown function', () => {
      const onKeyDownFunction = jest.fn();
      wrapper = render({ onKeyDown: onKeyDownFunction });
      wrapper.simulate('click');
      expect(onKeyDownFunction).toHaveBeenCalled();
    });
  });
