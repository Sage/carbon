...
This component uses context to detect if an input within a `Tab` has a validation error or warning. If so, the `Tab` will render with the corresponding validation state.
