...
import { StyledPicklist, StyledEmptyContainer } from './duelling-picklist.style';
...
export const Picklist = ({ disabled, children, placeholder }) => {
  ...
  }, () => React.createRef()), [children]);

  const focusItem = useCallback((ev, index) => {
    ev.preventDefault();
    if (index === -1) {
  ...
  const content = React.Children.map(children, (child, index) => React.cloneElement(child, {
    ref: refs[index],
    index,
    disabled,
