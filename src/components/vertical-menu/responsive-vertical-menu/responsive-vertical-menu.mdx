...
The `icon` and `customIcon` props are optional; if you wish to render a [Carbon icon](../?path=/story/foundations-icon--list-of-icons), use the `icon` property. Custom icons, such as `SVG` graphics,
...
<Canvas of={ResponsiveVerticalMenuStories.ItemWithCustomLabel} />
