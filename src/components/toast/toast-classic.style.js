...
const classicToastTypeStyle = ({ theme, variant }) =>
  isClassic(theme) &&
  css`
    align-items: center;
    display: flex;
...
const classicToastStyle = ({ theme, variant }) =>
  isClassic(theme) &&
  css`
    margin-top: 30px;
    position: fixed;
    ...
    &.toast-enter.toast-enter-active {
      opacity: 1;
      margin-top: 30px;
      transition: all 300ms 1000ms cubic-bezier(0.25, 0.25, 0, 1.5);
    }

    &.toast-exit.toast-exit-active {
      opacity: 0;
      right: -360px;
      transition: all 300ms cubic-bezier(0.96, -0.335, 0.75, 0.75);
    }
...
const classicToastContentStyle = ({ theme }) =>
  isClassic(theme) &&
  css`
