...
<Preview>
  <Story name="stacked-delayed" parameters={{ info: { disable: true }, chromatic: { disable: true }}} >
  {() => {
...
<Preview>
  <Story name="stacked" parameters={{ info: { disable: true }, chromatic: { disable: true }}} >
  {() => {
    const onDismissClickA = (evt) => {
      setIsOpenA(!isOpenA);
      action('click')(evt);
...
<Preview>
  <Story name="default" parameters={{ info: { disable: true }, chromatic: { disable: true }}} >
  {() => {
    const [isOpen, setIsOpen] = useState(false);
    const handleToggle = () => {
      if (!isOpen) {
