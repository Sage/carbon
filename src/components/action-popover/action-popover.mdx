...
A menu item can be passed a sub-menu to add further sub-actions if needed. Items with sub-menus will display chevron
...
leave the focus on the current item.
...
When a row has many actions you may choose to include the primary actions with a dedicated action for less frequently
