...
test.each([
  ...
  async (keyName, key) => {
    ...
    const header = screen.getByTestId("accordion-wrapper");
    header.focus();
...
test("applies expected styles when expanded when the variant is `subtle`", () => {
  ...
  const header = screen.getByTestId("accordion-summary-title-wrapper");
  expect(header).toHaveStyleRule("margin-bottom", "var(--spacing200)");
...
test("renders a small accordion with the correct spacing values applied", () => {
  ...
  expect(screen.getByTestId("accordion-wrapper")).toHaveStyleRule(
    "padding",
    "var(--spacing200)",
  );
});
