...
interface StyledWrapperProps
  extends Pick<AccordionProps, "borders" | "size" | "variant" | "width">,
    MarginProps {}
...
const DEFAULT_MAX_HEIGHT_TRANSITION_MS = "100ms";
const DEFAULT_PADDING_TRANSITION_MS = "50ms";
const DEFAULT_ROTATE_TRANSITION_MS = "200ms";

const getTransitionMs = (reduceMotion = false, defaultMs: string) => {
  return reduceMotion ? "0ms" : defaultMs;
};
...
const StyledWrapper = styled.div<StyledWrapperProps>`
  ...
  padding: ${({ size, variant }) => calculatePadding(size, variant)};
...
const StyledDetails = styled.details<StyledDetailsProps>`
  ...
  &[open] + div.content {
    ...
    transition:
      max-height
        ${({ reduceMotion }) =>
          getTransitionMs(reduceMotion, DEFAULT_MAX_HEIGHT_TRANSITION_MS)}
        ease-in-out,
      padding
        ${({ reduceMotion }) =>
          getTransitionMs(reduceMotion, DEFAULT_PADDING_TRANSITION_MS)}
        linear;
  }
  ...
  span {
    ...
    transition: rotate
      ${({ reduceMotion }) =>
        getTransitionMs(reduceMotion, DEFAULT_ROTATE_TRANSITION_MS)}
      ease-in;
  }
