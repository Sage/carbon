...
value like in the example below, although the component will also work if you use the `rawValue`. The `rawValue` should be 
used for storing the value in the backend and for running validations: it will either return a valid ISO string or null.
