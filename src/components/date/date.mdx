...
The `typical` variant is the default. Deprecated props remain part of the shared API to avoid a breaking type change, but the following legacy presentation props are no longer supported by either variant.

| Deprecated prop           | Behavior                                             | Replacement                                               |
| ------------------------- | ---------------------------------------------------- | --------------------------------------------------------- |
| `adaptiveLabelBreakpoint` | Not supported; layout is controlled by `labelInline` | Handle responsive layout in the implementation            |
| `fieldHelp`               | Not supported                                        | `inputHint`                                               |
| `labelHelp`               | Not supported                                        | `inputHint`                                               |
...
(`zh-CN`), Polish (`pl-PL`), and British English (`en-GB`) locale configuration. Locale values are merged with `en-GB`, so omitted
keys fall back to British English. Applications serving users in another language should provide approved translations through
`I18nProvider`, including the calendar controls announced by assistive technology: `openCalendarButton`, `previousMonthButton`,
`nextMonthButton`, `chooseMonth`, `chooseYear`, and `closeButton`.
