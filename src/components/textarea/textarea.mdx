...
### Size
Use the `size` prop to specify the `Textarea` size. This influences the typography settings and the spacing of the component's internal elements.

<Canvas of={TextareaStories.SizeStory} />

### Resize
Use the `resize` prop to enable resizing of the `Textarea`. This behaviour is disabled in the legacy validation.

<Canvas of={TextareaStories.ResizeStory} />
