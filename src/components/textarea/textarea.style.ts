...
import { TextareaProps, getDefaultMinHeightBySize } from "./textarea.component";
...
const StyledTextarea = styled.div.attrs(applyBaseTheme)<StyledTextAreaProps>`
  ...
  textarea[data-element="input"] {
    ...
    min-height: ${({ minHeight, $size }) =>
      minHeight ? `${minHeight}px` : getDefaultMinHeightBySize($size)};
    padding: ${({ $size }) =>
  ...
  ${InputPresentationStyle} {
    ...
    ${({ $hasError }) =>
      $hasError &&
      `box-shadow: inset 0 0 0 1px var(--input-validation-border-error);
        border-color: var(--input-validation-border-error) !important;`}
  ...
  ${InputPresentationStyle}:focus-within {
    ${addFocusStyling()}
    z-index: 2;
  }
  ...
  ${({ $labelAlign }) => css`
    ${StyledHintText} {
      justify-content: ${$labelAlign !== "right" ? "flex-start" : "flex-end"};
    }
  `};
...
export const StyledTextareaLabelContainer = styled.div<StyledTextareaLabelContainerProps>`
  ...
  ${({ $labelInline, $labelAlign, $labelSpacing, $labelWidth, $size }) => {
    ...
    const resolvedSpacing = $labelInline
      ? $labelSpacing
        ? `var(${$labelSpacing === 1 ? "--spacing100" : "--spacing200"})`
        : $size === "large"
