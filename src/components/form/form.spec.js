  ...
  describe('FormSummary', () => {
    ...
    describe('when warningCount prop is set on Form', () => {
      it('renders warning summary with properly colored icon and text', () => {
        ...
        expect(warningSummary.find('span').at(1).text()).toBe('There is 1 warning');
    ...
    describe('when either errorCount or warningCount or both are set on Form', () => {
      it.each([
        ...
        (errorCount, warningCount, errMessage, warnMessage) => {
          ...
          if (errorCount) {
            expect(wrapper.find(StyledInternalSummary).at(0).text()).toBe(errMessage);
          }
          if (warningCount) {
            expect(wrapper.find(StyledInternalSummary).at(warningPosition).text()).toBe(warnMessage);
          }
        }
      );
    });
