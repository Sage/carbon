...
describe("Table", () => {
  ...
  describe("selectRow", () => {
    ...
    describe("if there is a actionToolbarComponent", () => {
      it("calls setState on the action toolbar", () => {
        ...
        expect(spy).toHaveBeenCalledWith({
          total: 1,
          selected: instance.selectedRows,
        });
      });
    });
    ...
    describe("if multi select", () => {
      ...
      describe("if row is to be selected but is selectAll", () => {
        it("does not add the row", () => {
          instance = TestUtils.renderIntoDocument(
            <Table onSelect={spy} selectable>
              <TableRow uniqueID="foo" selectAll />
            </Table>
          );
          row = TestUtils.findRenderedComponentWithType(instance, TableRow);
          instance.selectRow("foo", row, true);
    ...
    describe("if there is an onHighlight callback", () => {
      it("calls the callback", () => {
        spy = jasmine.createSpy();
        instance = TestUtils.renderIntoDocument(
          <Table highlightable onHighlight={spy}>
            <TableRow uniqueID="foo" />
          </Table>
        );
        row = TestUtils.findRenderedComponentWithType(instance, TableRow);
  ...
  describe("selectAll", () => {
    ...
    it("calls selectRow for item in row array", () => {
      ...
      expect(instance.selectRow).toHaveBeenCalledWith(
        "foo",
        instance.rows.foo,
        true,
        true
    ...
    describe("if there is an onSelect callback", () => {
      it("calls the callback", () => {
        spy = jasmine.createSpy();
        instance = TestUtils.renderIntoDocument(<Table onSelect={spy} />);
        instance.rows = {};
        row = { state: {}, setState: () => {} };
        instance.selectAll(row);
        expect(spy).toHaveBeenCalledWith({});
    ...
    describe("when one of the rows cannot be selected", () => {
      it("only selects rows that can be selected", () => {
        ...
        instance.rows = {
          foo: {
            props: { uniqueID: "foo" },
            shouldHaveMultiSelectColumn: false,
          },
          bar: {
            props: { uniqueID: "bar" },
            shouldHaveMultiSelectColumn: true,
          },
        };

        spyOn(instance, "selectRow");
        instance.selectAll(row);
        expect(instance.selectRow).not.toHaveBeenCalledWith(
          "foo",
          instance.rows.foo,
          true,
          true
  ...
  describe("componentWillReceiveProps", () => {
    ...
    beforeEach(() => {
      ...
      instance = TestUtils.renderIntoDocument(<Table filter={data} />);
      spyOn(instance, "emitOnChangeCallback");
    });
    ...
    describe("when highlightable is disabled", () => {
      it("resets the highlighted row", () => {
        instance = TestUtils.renderIntoDocument(<Table highlightable />);
        spyOn(instance, "resetHighlightedRow");
    ...
    describe("when selectable is disabled", () => {
      it("resets the selectable rows", () => {
        instance = TestUtils.renderIntoDocument(<Table selectable />);
        instance.rows = {
  ...
  describe("resizeTable", () => {
    ...
    describe("when shrink is enabled", () => {
      beforeEach(() => {
        instance = TestUtils.renderIntoDocument(<Table shrink />);
      });
  ...
  describe("shouldResetTableHeight", () => {
    describe("when new page size is smaller than previous", () => {
      it("returns true", () => {
        ...
        expect(instancePager.shouldResetTableHeight(prevProps, {})).toEqual(
          true
        );
      });
    });
  ...
  describe("onConfigure", () => {
    ...
    beforeEach(() => {
      ...
      wrapper = mount(
        <Table className="foo" onConfigure={onConfigureSpy}>
          <TableRow />
        </Table>
      );
    });
  ...
  describe("thead", () => {
    describe("when thead is not provided", () => {
      it("returns the the correct markup", () => {
        instance = TestUtils.renderIntoDocument(<Table path="/test" />);
        const parent = TestUtils.scryRenderedDOMComponentsWithTag(
          instance,
          "thead"
        )[0];
    ...
    describe("when thead is provided", () => {
      it("returns the the correct markup", () => {
        ...
        expect(instance.thead).toEqual(<thead>{header}</thead>);
      });
    });
  });
  ...
  describe("tableContent", () => {
    ...
    describe("if there are children that are immutable", () => {
      ...
      it("returns the children with the loading row if only row is a header and has not yet received data", () => {
        ...
        expect(instance.tableContent.get(0)).toEqual(
          instance.props.children.get(0)
        );
      ...
      describe("when a customEmptyRow has been provided", () => {
        test("the custom empty row is rendered instead of the default", () => {
          ...
          const customEmptyRow = (
            <TableRow className="phil">Custom Empty Row</TableRow>
          );
          instance = TestUtils.renderIntoDocument(
            <Table customEmptyRow={customEmptyRow}>{children}</Table>
          );
          instance._hasRetreivedData = true;
  ...
  describe("render", () => {
    ...
    it("renders a caption tag when a caption prop is given", () => {
      const wrapper = shallow(<Table caption="Acme widgets" />);
  ...
  describe("isPassive", () => {
    it("returns false if isPassiveData and highlightable are true and selectable is false or undefined", () => {
      instance = TestUtils.renderIntoDocument(
        <Table isPassiveData highlightable />
      );
    ...
    it("returns false if isPassiveData and selectable are true and highlightable is false or undefined", () => {
      instance = TestUtils.renderIntoDocument(
        <Table isPassiveData selectable />
      );
      expect(instance.isPassive).toEqual(false);
    });
    ...
    it("returns false if isPassiveData, selectable and highlightable are true", () => {
      instance = TestUtils.renderIntoDocument(
        <Table isPassiveData selectable hightlightable />
      );
      expect(instance.isPassive).toEqual(false);
    });
  ...
  describe("tags on component", () => {
    it("include correct component, element and role data tags", () => {
      const wrapper = shallow(
        <Table data-element="bar" data-role="baz" path="test">
          <TableRow />
        </Table>
      );
  ...
  describe("theme", () => {
    describe.each([
      ...
      "tertiary",
      "transparent",
    ])("when the table type is %s", (type) => {
      it("renders a table to match the expected style", () => {
        const wrapper = mount(
          <StyledTable isZebra theme={mintTheme} tableType={type} />
        );
  ...
  describe.each(["compact", "small"])("when the table size is %s", (size) => {
    it("renders a table to match the expected style", () => {
      const wrapper = TestRenderer.create(
        <StyledTable isZebra theme={mintTheme} size={size} />
      );
