...
describe("TableHeader", () => {
  ...
  beforeEach(() => {
    ...
    instance = TestUtils.renderIntoDocument(
      <Table>
        <TableRow>
          <TableHeader align="right" style={{ width: "50px" }} />
        </TableRow>
      </Table>
    );
    ...
    sortableColumn = TestUtils.findRenderedDOMComponentWithTag(
      instanceSortable,
      "th"
    );
    sortableHeader = TestUtils.scryRenderedComponentsWithType(
      instanceSortable,
      TableHeader
    )[0];
  });
  ...
  describe("emitSortEvent", () => {
    describe("if no sortOrder has been specified", () => {
      it("calls the tables onSort function with the default params", () => {
        ...
        expect(instanceSortable.props.onChange).toHaveBeenCalledWith("table", {
          currentPage: 0,
          pageSize: "",
          filter: {},
          sortedColumn: "name",
  ...
  describe("sortable columns", () => {
    describe("if a column is sortable", () => {
      ...
      describe("aria-label", () => {
        ...
        it("states the column is sortable, and activating will sort descending", () => {
          ariaLink = wrapper.find(
            'a[aria-label="Sortable column, activate to sort column descending"]'
          );
        ...
        it("includes the current sort order after sorting", () => {
          ...
          ariaLink = wrapper.find(
            'a[aria-label="Sortable column, sorted descending, activate to sort column ascending"]'
          );
          expect(ariaLink.exists()).toBe(true);
        });
      });
      ...
      describe("before a sortable header is clicked", () => {
        ...
        it("contains a link with a descriptive aria-label that defaults to descending", () => {
          expect(ariaLink.getAttribute("aria-label")).toEqual(
            "Sortable column, activate to sort column descending"
          );
        });
      });
    ...
    describe("if a column is not sortable", () => {
      it("does not return an icon", () => {
        const nonSortableHeader = TestUtils.scryRenderedComponentsWithType(
          instance,
          TableHeader
        )[0];
    ...
    describe("sortDescription", () => {
      describe("when a column is sortable", () => {
        ...
        it('says the column is a "Sortable column, activate to sort column descending"', () => {
          expect(wrapper.instance().sortDescription).toEqual(
            "Sortable column, activate to sort column descending"
          );
        });
        ...
        it('includes "sorted descending"  when the current sort order is "desc"', () => {
          ...
          expect(wrapper.instance().sortDescription).toEqual(
            "Sortable column, sorted descending, activate to sort column ascending"
          );
        });
        ...
        it('includes "sorted ascending"  when the current sort order is "asc"', () => {
          ...
          expect(wrapper.instance().sortDescription).toEqual(
            "Sortable column, sorted ascending, activate to sort column descending"
          );
        });
      });
  ...
  describe("render", () => {
    ...
    it("renders a th to match the expected default style when a width is provided", () => {
      wrapper = mount(
        <Table>
          <TableRow>
            <TableHeader align="right" sortable width="200" />
          </TableRow>
        </Table>
      );
      th = wrapper.find("th").hostNodes();
