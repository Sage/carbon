      ...
      <TableBody>
        {sortedProducts.map(({ id, product, price, status }) => (
          <TableRow key={id} id={`interaction-sort-row-${id}`}>
            <TableCell id={`interaction-sort-row-${id}-product`}>
              {product}
            </TableCell>
            <TableCell
              id={`interaction-sort-row-${id}-price`}
            >{`£${price}`}</TableCell>
            <TableCell id={`interaction-sort-row-${id}-status`}>
              {status}
            </TableCell>
          </TableRow>
        ))}
        ...
        >
          <TableHeader id="interaction-select-header-checkbox" width="80px">
            <Checkbox
              ...
              indeterminate={
                selectedRows.length > 0 && selectedRows.length < rowIds.length
              }
      ...
      <TableBody>
        {rowIds.map((rowId, index) => (
          ...
          >
            ...
            <TableCell
              id={`interaction-select-row-${index + 1}-product`}
            >{`Product ${index + 1}`}</TableCell>
            <TableCell id={`interaction-select-row-${index + 1}-status`}>
  ...
  <Table variant="prominent">
    ...
    <TableBody>
        ...
        subRows={
          <>
            <TableRow id="interaction-expand-child-1">
              <TableCell id="interaction-expand-child-1-product">
                Product A1
              </TableCell>
              <TableCell id="interaction-expand-child-1-type">
                Child product
              </TableCell>
              <TableCell id="interaction-expand-child-1-status">
                Active
              </TableCell>
            </TableRow>
            <TableRow id="interaction-expand-child-2">
              <TableCell id="interaction-expand-child-2-product">
                Product A2
              </TableCell>
              <TableCell id="interaction-expand-child-2-type">
                Child product
              </TableCell>
              <TableCell id="interaction-expand-child-2-status">
                Inactive
              </TableCell>
            </TableRow>
          </>
      ...
      >
        ...
        <TableCell id="interaction-expand-parent-type">
          Parent product
        </TableCell>
      ...
      <TableRow id="interaction-expand-sibling">
        ...
        <TableCell id="interaction-expand-sibling-type">
          Standard product
        </TableCell>
