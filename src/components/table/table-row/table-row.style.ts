import styled, { css } from "styled-components";
import { BorderThickness } from "../table.component";
import borderThicknessStyles from "../__internal__/config";

interface StyledTableRowProps {
  $isSelected?: boolean;
  $borderThickness?: BorderThickness;
  $dropIndicatorPosition?: "top" | "bottom" | null;
  $isDraggable?: boolean;
  $isDragging?: boolean;
  $isSubRowVisible?: boolean;
  $shouldClipSubRow?: boolean;
}

const StyledTableRow = styled.tr<StyledTableRowProps>`
  ${({ $borderThickness }) =>
    $borderThickness &&
    css`
      --table-cell-border-horizontal-width: ${borderThicknessStyles[
        $borderThickness
      ]};
    `}

  ${({ $dropIndicatorPosition }) =>
    $dropIndicatorPosition &&
    css`
      &&& {
        > th,
        > td {
          position: relative;
        }

        > th::before,
        > td::before {
          content: "";
          position: absolute;
          z-index: 10;
          right: -1px;
          top: ${$dropIndicatorPosition === "top" ? "0" : "auto"};
          bottom: ${$dropIndicatorPosition === "bottom" ? "0" : "auto"};
          left: -1px;
          height: 3px;
          background-color: var(--container-action-target-bg-default);
          pointer-events: none;
        }
      }
    `}

  ${({ $isDragging }) =>
    $isDragging &&
    css`
      && {
        position: relative;
        z-index: 2;
        box-shadow: var(--global-depth-lvl3);

        &&& {
          > th:not(:last-child),
          > td:not(:last-child) {
            border-right-color: transparent;
            box-shadow: none;
          }

          > th:last-child,
          > td:last-child {
            border-left-color: transparent;
            box-shadow: none;
          }

          > th,
          > td {
            background-color: var(--container-draggable-ghost);
            background-color: color-mix(
              in srgb,
              var(--container-draggable-ghost) 50%,
              transparent
            );

            * {
              visibility: hidden;
            }
          }

          > th::before,
          > th::after,
          > td::before,
          > td::after {
            display: none;
          }
        }
      }
    `}

  ${({ $isSelected }) =>
    $isSelected &&
    css`
      > td,
      > th {
        background-color: var(--table-row-bg-selected);
      }
    `}

  ${({ $isSubRowVisible }) =>
    $isSubRowVisible !== undefined &&
    css`
      &[data-component$="sub-row"] {
        ${$isSubRowVisible &&
        css`
          &:not([data-component$="sub-row"] + [data-component$="sub-row"]) {
            > td,
            > th {
              position: relative;
            }

            td::before,
            th::before {
              content: "";
              position: absolute;
              top: 0;
              left: 0;
              right: 0;
              height: 2px;
              background: linear-gradient(
                90deg,
                transparent 0%,
                var(--mode-color-generic-depth-faint) 0%,
                var(--mode-color-generic-depth-faint) 100%,
                transparent 100%
              );
              pointer-events: none;
            }
          }
        `}

        && {
          > td,
          > th {
            height: ${$isSubRowVisible ? "auto" : "0"};
            padding: 0;
            ${!$isSubRowVisible &&
            css`
              border-bottom-width: 0;
            `}
          }
        }

        > td > [data-element="table-cell-collapse"],
        > th > [data-element="table-cell-collapse"] {
          grid-template-rows: ${$isSubRowVisible ? "1fr" : "0fr"};
          opacity: ${$isSubRowVisible ? 1 : 0};
          transform: ${$isSubRowVisible ? "none" : "translateY(-4px)"};
        }

        @media (prefers-reduced-motion: no-preference) {
          > td > [data-element="table-cell-collapse"],
          > th > [data-element="table-cell-collapse"] {
            transition:
              grid-template-rows 200ms ease,
              opacity 150ms ease,
              transform 200ms ease;
          }
        }
      }
    `}

  ${({ $shouldClipSubRow }) =>
    $shouldClipSubRow !== undefined &&
    css`
      > td
        > [data-element="table-cell-collapse"]
        > [data-element="table-cell-clip"],
      > th
        > [data-element="table-cell-collapse"]
        > [data-element="table-cell-clip"] {
        overflow: ${$shouldClipSubRow ? "hidden" : "visible"};
      }
    `}
`;

export default StyledTableRow;
