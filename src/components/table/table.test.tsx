...
describe("drag and drop", () => {
  ...
  it("renders the drop indicator below the target when dragging downwards", () => {
    renderTable({
      isDraggable: true,
      rows: [
        createRow("one", "Row one"),
  ...
  it("renders the drop indicator above the target when dragging upwards", () => {
    renderTable({
      isDraggable: true,
      rows: [
        createRow("one", "Row one"),
    ...
    expect(dropTarget).toHaveAttribute("data-drop-indicator-position", "top");
...
describe("expandable rows", () => {
  ...
  it("allows content to overflow when a sub row is fully expanded", () => {
    renderTable({
      rows: createRow("one", "Parent row", {
        isExpanded: true,
        subRows: createRow("one-child", "Child row"),
    ...
    const subRow = screen.getByRole("row", { name: "Child row" });
    const subRowCell = within(subRow).getByRole("cell", { name: "Child row" });
    // eslint-disable-next-line testing-library/no-node-access
    const clipContainer = subRowCell.querySelector<HTMLElement>(
      '[data-element="table-cell-clip"]',
    );

    expect(subRowCell).toHaveStyle({ height: "auto" });
    expect(clipContainer).toHaveStyle({ overflow: "visible" });
  });
});
...
describe("zebra striping", () => {
  it("applies the alternate background to even non-sub rows when enabled", () => {
    ...
    ["td", "th"].forEach((cellSelector) => {
      expect(screen.getByRole("table")).toHaveStyleRule(
        "background-color",
        "var(--table-row-bg-alt)",
        {
          modifier: `tbody tr:nth-child(even of :not([data-component*="sub-row"])):not( [data-is-selected="true"] ) ${cellSelector}`,
        },
      );
    });
  });
...
describe("table props", () => {
  it("centre-aligns table header and table cell content", () => {
    renderTable({
      ...
      rows: createRow("one", "Row one", {}, { align: "center" }),
    });

    const headerContent = within(
      screen.getByRole("columnheader", { name: "Header row" }),
    ).getByText("Header row");
    const cellContentContainer = screen
  ...
  it("updates the scroll container tab stop when its overflow changes", () => {
    ...
    const scrollContainer = screen
      .getByTestId("table-inner-wrapper")
      // eslint-disable-next-line testing-library/no-node-access
      .querySelector<HTMLElement>('[data-element="table-scroll-container"]');

    expect(scrollContainer).toHaveAttribute("tabindex", "0");
    expect(observe).toHaveBeenCalledWith(scrollContainer);
    expect(observe).toHaveBeenCalledWith(screen.getByRole("table"));
  ...
  it("uses the measured sticky header and footer heights as focus scroll margins", () => {
    let resizeCallback: ResizeObserverCallback;
    const heights = { header: 48, footer: 40 };
    const observe = jest.fn();
    const disconnect = jest.fn();
    const resizeObserver = {
      observe,
    ...
    const offsetHeightSpy = jest
      ...
      .mockImplementation(function (this: HTMLElement) {
        if (this.tagName === "THEAD") return heights.header;
        if (this.tagName === "TFOOT") return heights.footer;
        return 0;
      });
    ...
    expect(table).toHaveStyleRule(
      ...
      "calc( var(--table-sticky-header-height) + var(--global-space-comp-xs) )",
      {
        modifier: '&[data-has-sticky-header="true"] tbody :focus',
      },
    );
    expect(table).toHaveStyleRule(
      "scroll-margin-block-end",
      "calc( var(--table-sticky-footer-height) + var(--global-space-comp-xs) )",
      {
        modifier: '&[data-has-sticky-footer="true"] tbody :focus',
      },
    );
  ...
  it.each([
    ["header", 20, 36, "start"],
    ["footer", 150, 170, "end"],
    ["both", 80, 96, undefined],
  ] as const)(
    "scrolls a focused body control clear of sticky %s rows when needed",
    (stickyRow, controlTop, controlBottom, expectedBlock) => {
      const resizeObserverSpy = jest
        .spyOn(window, "ResizeObserver")
        .mockImplementation(
          () =>
      ...
      const { unmount } = renderTable({
        headerRows: createHeaderRow("header", "Header row"),
        rows: (
          <TableRow id="selectable-row">
            <TableCell id="selectable-cell">
              {stickyRow === "footer" ? (
                <button type="button">Focusable control</button>
              ) : (
                <input aria-label="Focusable control" />
              )}
            </TableCell>
          </TableRow>
        ),
      ...
      const control = screen.getByRole(
        stickyRow === "footer" ? "button" : "textbox",
        { name: "Focusable control" },
      );
      const header = screen.getByTestId("table-head");
      const footer = screen.getByTestId("table-footer");
      ...
      jest
        ...
        .mockReturnValue(createBounds(controlTop, controlBottom));
      jest
        .spyOn(header, "getBoundingClientRect")
        .mockReturnValue(createBounds(0, 40));
      jest
        .spyOn(footer, "getBoundingClientRect")
        .mockReturnValue(createBounds(160, 200));

      fireEvent.focus(control);

      expect(scrollIntoView.mock.calls).toEqual(
        expectedBlock ? [[{ block: expectedBlock, inline: "nearest" }]] : [],
      );
  ...
  it("resets measured sticky row heights when sticky rows are removed", () => {
    const resizeObserver = {
      observe: jest.fn(),
      unobserve: jest.fn(),
      disconnect: jest.fn(),
    ...
    const resizeObserverSpy = jest
      .spyOn(window, "ResizeObserver")
      .mockImplementation(() => resizeObserver);
    const offsetHeightSpy = jest
      .spyOn(HTMLElement.prototype, "offsetHeight", "get")
      .mockImplementation(function (this: HTMLElement) {
        if (this.tagName === "THEAD") return 48;
        if (this.tagName === "TFOOT") return 40;
        return 0;
      });
    const testTable = (stickyRow?: TableProps["stickyRow"]) => (
      <Table stickyRow={stickyRow}>
        <TableHead>{createHeaderRow("header", "Header row")}</TableHead>
        <TableBody>{createRow("one", "Row one")}</TableBody>
        <TableFoot>{createRow("footer", "Footer row")}</TableFoot>
      </Table>
    );
    const { rerender, unmount } = render(testTable("both"));
    const table = screen.getByRole("table");
  ...
  it.each([
    ...
    "applies the %s size to body cells",
    (size, expectedMinHeight) => {
      renderTable({
        rows: createRow("one", "Row one"),
        tableProps: { size },
      });
  ...
  it("allows cell content to fill the height established by the table row", () => {
    renderTable({
      rows: createRow("one", "Parent row", {
        subRows: createRow("one-child", "Child row"),
      }),
    ...
    const cell = screen.getByRole("cell", { name: "Parent row" });
    // eslint-disable-next-line testing-library/no-node-access
    const collapseContainer = cell.querySelector<HTMLElement>(
      '[data-element="table-cell-collapse"]',
    );
    // eslint-disable-next-line testing-library/no-node-access
    const clipContainer = cell.querySelector<HTMLElement>(
      '[data-element="table-cell-clip"]',
    );
    // eslint-disable-next-line testing-library/no-node-access
    const contentContainer = cell.querySelector<HTMLElement>(
      '[data-element="table-cell-content-container"]',
    );

    expect(cell).toHaveStyle({ position: "relative" });
    expect(collapseContainer).toHaveStyle({
      position: "absolute",
      inset: 0,
    });
    expect(clipContainer).toHaveStyle({ height: "100%" });
  ...
  it.each([
    ...
    ["large", "var(--global-size-l)"],
    ["extra-large", "var(--global-size-xxl)"],
  ] as const)("applies the %s size to header cells", (size, expectedHeight) => {
    renderTable({
      headerRows: createHeaderRow("header", "Header row"),
      rows: createRow("one", "Row one"),
      tableProps: { size, variant: "subtle-white" },
    });

    expect(
      screen.getByRole("columnheader", { name: "Header row" }),
    ).toHaveStyleRule("height", expectedHeight);
  });
...
describe("with sortable columns", () => {
  it("right-aligns non-sortable header content and sortable header buttons", () => {
    renderTable({
      headerRows: [
        createHeaderRow(
          "non-sortable-header",
        ...
        createHeaderRow(
          ...
          "Sortable header",
          {},
          { align: "right", onSort: jest.fn(), sortType: "unsorted" },
        ),
      ],
      rows: createRow("one", "Row one"),
    });

    const nonSortableHeader = screen.getByRole("columnheader", {
      name: "Non-sortable header",
    });
    const nonSortableContent = within(nonSortableHeader).getByText(
      "Non-sortable header",
    );
    const sortableHeader = screen.getByRole("columnheader", {
      name: "Sortable header",
    });
    const sortButton = within(sortableHeader).getByRole("button");

    expect(nonSortableContent).toHaveStyle("justify-content: flex-end");
  ...
  it.each<TableHeaderProps["sortType"]>([
    ...
    "descending",
    "unsorted",
  ])("renders the expected sort icon when sortType is %s", (sortType) => {
    renderTable({
      headerRows: createHeaderRow(
        "header",
        "Header row",
