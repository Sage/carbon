...
testStyledSystemMargin((props) => (
  <ControlledColorPicker name="advancedPicker" {...props} />
));
...
test.each(availableColors)(
  ...
  async ({ value, label }) => {
    ...
    const styleProperty =
      value === "transparent" ? "backgroundImage" : "backgroundColor";
    const styleValue =
      value === "transparent"
        ? `#eee url("data:image/svg+xml,${transparentSvg}")`
        : value;
    expect(screen.getByRole("button", { name: "Change colour" })).toHaveStyle({
...
test("when a color input is clicked, it triggers the onBlur callback", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "Change colour" }));
  // need to run timers to allow the dialog animation to complete, without which the selected color won't be focused
  jest.runAllTimers();
  ...
  // To work around this, we fire 2 clicks - the first of these can be removed, and the assertion changed to assert that "orchid" was blurred,
...
test("when the 'open' prop is true, the dialog is open on mount", () => {
