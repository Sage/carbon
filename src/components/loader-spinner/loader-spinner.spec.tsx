...
describe("LoaderSpinner", () => {
  ...
  describe("when custom props are passed", () => {
    it.each(sizes)(
      "when the 'size' prop is passed as `%s` the wrapper has the correct dimensions",
      (spinnerSizes) => {
    ...
    it.each(sizes)(
      "when the 'size' prop is passed as `%s` the wrapper has the correct minimum dimensions",
      (spinnerSizes) => {
        render(<LoaderSpinner size={spinnerSizes} />);
        const wrapperElement = screen.getByRole("status");
