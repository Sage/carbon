...
export const Typography = ({
  ...
  function calculateAs() {
    ...
    // added guard here to ensure only semantic HTML elements are rendered when an unsupported variant (e.g. "foo") is used without an "as" prop
    if ((VARIANT_TYPES as readonly string[]).includes(variant)) {
      return variant;
    }
    return "p";
  }
  ...
  return (
    <StyledTypography
      id={id}
      as={calculateAs()}
      // all props which are not valid HTML attributes are prefixed with "$" to prevent them from being passed to the DOM,
      ...
      // prefixing allowed CSS text override props with "$" to prevent them from being passed to the DOM
      {...Object.fromEntries(
        Object.entries(
