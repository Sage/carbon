...
The `color` prop applies token-based text colours.
Available options are `"neutral"`, `"subtle"`, `"caution"`, `"info"`, `"negative"`, and `"positive"`.
Available on all variants, including heading variants (`"h1"` to `"h5"`, `"section-heading"`, and `"section-subheading"`).

<Canvas of={TypographyStories.ColorStory} />
