...
You can use the `CreateEmpty` function to create an empty editor state. This function should be called with no arguments, and the result should be passed to the `initialValue` property of the editor.
...
Providing an `onCancel` callback causes the editor to render a cancel button. When clicked, `onCancel` will be called and the editor will reset to its original content provided on initial render.
...
>**Note**: `TextEditor` is an uncontrolled component, therefore passing any updated content from `onChange` to the `initialValue` prop will not update the editor's content. See [this section](#externally-overwriting-the-editors-content) if you need to externally overwrite the editor's content.
