...
describe("StyledSpanNode", () => {
  ...
  describe("StyledSpanNode class", () => {
    ...
    test("should clone node correctly", () => {
      editor?.update(() => {
        const original = new StyledSpanNode(
          "Test Text",
          "700",
          "24px",
          "30px",
    ...
    test("should create node from typography key", () => {
      editor?.update(() => {
        ...
        expect(titleNode.getFontSize()).toBe("30px");
        expect(titleNode.getLineHeight()).toBe("37.5px");
      });
    });
  ...
  describe("Getters and Setters", () => {
    let node: StyledSpanNode;

    beforeEach(() => {
      editor?.update(() => {
        node = new StyledSpanNode("Test", "400", "14px", "21px");
      });
    });

    test("should get font weight correctly", () => {
      expect(node.getFontWeight()).toBe("400");
    });

    test("should get font size correctly", () => {
      expect(node.getFontSize()).toBe("14px");
    });

    test("should get line height correctly", () => {
  ...
  describe("Typography key detection", () => {
    test("should detect title typography", () => {
      editor?.update(() => {
        const node = new StyledSpanNode("Title", "700", "30px", "37.5px");
    ...
    test("should detect subtitle typography", () => {
      editor?.update(() => {
        const node = new StyledSpanNode("Subtitle", "700", "24px", "30px");
    ...
    test("should detect sectionHeader typography", () => {
      editor?.update(() => {
        const node = new StyledSpanNode("Header", "700", "21px", "26.25px");
    ...
    test("should detect sectionSubheader typography", () => {
      editor?.update(() => {
        const node = new StyledSpanNode("Subheader", "700", "18px", "22.5px");
  ...
  describe("DOM creation and manipulation", () => {
    test("should create DOM element with correct styles", () => {
      editor?.update(() => {
        ...
        expect(domElement).toHaveStyle({ fontWeight: "700" });
        expect(domElement).toHaveStyle({ fontSize: "24px" });
        expect(domElement).toHaveStyle({ lineHeight: "30px" });
    ...
    test("should update DOM when styles change", () => {
      editor?.update(() => {
        ...
        expect(domElement).toHaveStyle({ fontWeight: "700" });
        expect(domElement).toHaveStyle({ fontSize: "18px" });
        expect(domElement).toHaveStyle({ lineHeight: "24px" });
      });
    });
  ...
  describe("DOM export and import", () => {
    test("should export DOM correctly", () => {
      editor?.update(() => {
        ...
        expect(element).toHaveStyle({ lineHeight: "30px" });
        expect(element).toHaveTextContent("Export Test");
      });
    });
    ...
    test("should import from DOM element correctly", () => {
      editor?.update(() => {
        ...
        const { node } = conversionData.conversion();
  ...
  describe("Integration tests", () => {
    ...
    test("should serialize and deserialize correctly", () => {
      ...
      newEditor?.read(() => {
        ...
        const styledSpanNode = paragraph.getFirstChild();

        expect($isStyledSpanNode(styledSpanNode)).toBe(true);
        expect(styledSpanNode.getTextContent()).toBe("My Title");
        expect(styledSpanNode.getTypographyKey()).toBe("title");
        expect(styledSpanNode.getFontWeight()).toBe("700");
        expect(styledSpanNode.getFontSize()).toBe("30px");
        expect(styledSpanNode.getLineHeight()).toBe("37.5px");
      });
    });
    ...
    test("should handle style updates in editor context", () => {
      editor?.update(() => {
        ...
        // Update styles
        styledSpanNode.setFontWeight("700");
        styledSpanNode.setFontSize("24px");
        styledSpanNode.setLineHeight("30px");

        expect(styledSpanNode.getFontWeight()).toBe("700");
        expect(styledSpanNode.getFontSize()).toBe("24px");
        expect(styledSpanNode.getLineHeight()).toBe("30px");
