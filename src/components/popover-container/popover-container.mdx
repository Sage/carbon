...
Use the `roundness` prop to set the corner rounding of the `PopoverContainer` to either `moderate` or `curved`.

<Canvas of={PopoverContainerStories.Roundness} />

### Sizes

Use the `size` prop to set the size of the `PopoverContainer` to either `small`, `medium`, or `large`.

<Canvas of={PopoverContainerStories.Sizes} />
...
<TranslationKeysTable
  translationData={[
    {
      name: "popoverContainer.openButton.text",
      description: "The text for the button that opens the popover container",
      type: "func",
      returnType: "string",
    },
  ]}
