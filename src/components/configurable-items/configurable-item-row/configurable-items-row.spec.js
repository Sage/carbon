...
describe('ConfigurableItemRow', () => {
  ...
  describe('classNames', () => {
    beforeEach(() => {
      wrapper = shallow(<ConfigurableItemRowWithoutHOC className='my-custom-class-name' rowIndex={ 1 } />);
    ...
    describe('when the dragAndDropActiveIndex is the same as the rowIndex', () => {
      ...
      it('adds configurable-item-row--dragged to the classes', () => {
        expect(wrapper.find(ConfigurableItemRowStyle).prop('isDragged')).toEqual(true);
      });
    });
    ...
    describe('when the dragAndDropActiveIndex is the same as the rowIndex', () => {
      ...
      it('does not add configurable-item-row--dragged to the classes', () => {
        expect(wrapper.find(ConfigurableItemRowStyle).prop('isDragged')).toEqual(false);
      });
    });
  });
  ...
  describe('rowIndex', () => {
    beforeEach(() => {
      wrapper = shallow(
        <ConfigurableItemRowWithoutHOC rowIndex={ 2 } />
      );
    });
  ...
  describe('list item markup', () => {
    ...
    it('renders an <li> wrapped in WithDrop', () => {
      ...
      expect(withDrop.find('[data-element="configurable-item-row"]').length).toEqual(1);
    });
  });
});
