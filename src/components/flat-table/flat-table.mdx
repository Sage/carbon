...
```javascript
import { FlatTableBodyDraggableHandle } from "carbon-react/lib/components/flat-table";
```
The `FlatTableBodyDraggableHandle` type provides an imperative handle for control over the `FlatTableRow`'s movement. 
By passing a `ref` to the `FlatTableBodyDraggable` you can access its `reOrder()` method to set manually re-order elements. 
Pass two arguments: the `id` of the `FlatTableRow` you want to move and the destination index.
...
beyond simple top/bottom movements in your draggable rows.

This functionality gives users alternative ways to reposition elements when direct dragging isn't desired or possible.

<Canvas of={FlatTableStories.WithDraggableRowsAndManualReOrdering} />
