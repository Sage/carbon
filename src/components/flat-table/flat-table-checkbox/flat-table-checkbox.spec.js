...
describe('FlatTableCheckbox', () => {
  describe('the "as" prop is not passed in', () => {
    it('renders to match the expected styling for a "td" element', () => {
      expect(TestRenderer.create(<FlatTableCheckbox />).toJSON()).toMatchSnapshot();
    });

    it('has the correct "data-element" when rendered as a "td"', () => {
      expect(
        TestRenderer.create(<FlatTableCheckbox />).root.findByType(StyledFlatTableCheckbox).props['data-element']
  ...
  describe('"th" is passed in via the "as" prop', () => {
    it('renders to match the expected styling for a "th" element when it is passed via the "as" prop', () => {
      expect(TestRenderer.create(<FlatTableCheckbox as='th' />).toJSON()).toMatchSnapshot();
    });

    it('renders to match the expected styling for a "th" element', () => {
      expect(
        TestRenderer.create(<FlatTableCheckbox as='th' />).root
          .findByType(StyledFlatTableCheckbox).props['data-element']
  ...
  describe('"click" event', () => {
    it('calls "stopPropagation" when the "Checkbox" is clicked', () => {
      ...
      wrapper.root.findByType(Checkbox).props.onClick({ stopPropagation: mockClick });
