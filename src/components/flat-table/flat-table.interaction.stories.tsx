...
const FlatTableFocusStatesComponent = () => {
  ...
  const data = [
    { name: "John Doe", location: "London", status: "Single" },
    { name: "Jane Doe", location: "York", status: "Married" },
    { name: "John Smith", location: "Edinburgh", status: "Single" },
    { name: "Jane Smith", location: "Newcastle", status: "Married" },
  ];
  ...
  const renderRows = () => {
    ...
    const slice =
      start < 0
        ? data
        : data.slice(start, end > data.length ? data.length : end);

    return slice.map(({ name, location, status }, index) => (
      <FlatTableRow key={index}>
        <FlatTableCell>{name}</FlatTableCell>
        <FlatTableCell>{location}</FlatTableCell>
...
export const FlatTableFocusStates: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.type(secondRow, "{space}");
...
export const SelectableAndExpandableRows: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.tab();
    await userEvent.type(firstRowCheckbox, "{space}");
    ...
    await userEvent.tab();
    await userEvent.type(headerRowCheckbox, "{space}");
