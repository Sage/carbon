...
test("custom variant activates on Enter, focuses input, and deactivates on Escape", async () => {
  ...
  expect(input).toHaveStyle("height: var(--global-size-s)");
  expect(input).toHaveStyle("border-radius: var(--global-radius-action-m)");
...
test("custom variant uses the group aria-label when inputAriaLabel is not provided", async () => {
  const user = userEvent.setup();

  const WrappedCustomTile = () => {
    const [customValue, setCustomValue] = useState("");
  ...
  render(<WrappedCustomTile />);

  await user.click(screen.getByRole("button", { name: "Something else" }));

  expect(
    screen.getByRole("textbox", { name: "Payment options" }),
  ).toBeVisible();
});

test("custom variant falls back to the title when there is no inputAriaLabel or group", async () => {
  const user = userEvent.setup();

  const StandaloneCustomTile = () => {
    const [customValue, setCustomValue] = useState("");
  ...
  render(<StandaloneCustomTile />);

  await user.click(screen.getByRole("button", { name: "Something else" }));

  expect(screen.getByRole("textbox", { name: "Something else" })).toBeVisible();
});
...
test("renders the group legend as a labelset when provided and nothing else", () => {
  render(
    <OptionTileGroup
      selectionType="single"
      legend="Question tile"
      aria-label="Question tile"
    >
      <OptionTile variant="single" number={1} title="Pay now" />
    </OptionTileGroup>,
  );

  expect(screen.getByRole("group", { name: /Question tile/ })).toBeVisible();
  expect(screen.getByText("Question tile")).toBeVisible();
  expect(
    screen.queryByText("Choose how you would like to pay"),
  ).not.toBeInTheDocument();
});
