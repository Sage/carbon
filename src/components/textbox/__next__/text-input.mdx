...
```javascript
import TextInput from "carbon-react/lib/components/text-input";
```
...
- If content in a text input is never editable, think about removing the field border so it appears as static text.
...
<Canvas of={TextInputStories.WithCharacterLimit} />
