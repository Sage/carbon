...
describe("deprecation/rename messages", () => {
  ...
  it("logs a deprecation warning once when labelAlign prop is used", () => {
    const loggerSpy = jest.spyOn(Logger, "deprecate");
    render(
      <>
        <Textbox value="foo" onChange={() => {}} labelAlign="left" />
        <Textbox value="foo" onChange={() => {}} labelAlign="right" />
      </>,
    );
    expect(loggerSpy).toHaveBeenCalledWith(
      "The `labelAlign` prop has been deprecated and will soon be removed.",
    );
    const callsWithLabelAlignMessage = loggerSpy.mock.calls.filter((call) =>
      call[0].includes("The `labelAlign` prop has been deprecated"),
    );
    expect(callsWithLabelAlignMessage).toHaveLength(1);
    loggerSpy.mockRestore();
  });
  ...
  it("logs a deprecation warning once when labelSpacing prop is used", () => {
    const loggerSpy = jest.spyOn(Logger, "deprecate");
    render(
      <>
        <Textbox value="foo" onChange={() => {}} labelSpacing={1} />
        <Textbox value="foo" onChange={() => {}} labelSpacing={2} />
      </>,
    );
    expect(loggerSpy).toHaveBeenCalledWith(
      "The `labelSpacing` prop has been deprecated and will soon be removed.",
    );
    const callsWithLabelSpacingMessage = loggerSpy.mock.calls.filter((call) =>
      call[0].includes("The `labelSpacing` prop has been deprecated"),
    );
    expect(callsWithLabelSpacingMessage).toHaveLength(1);
    loggerSpy.mockRestore();
  });

  it("logs a deprecation warning once when reverse prop is used", () => {
    const loggerSpy = jest.spyOn(Logger, "deprecate");
    render(
      <>
        <Textbox value="foo" onChange={() => {}} reverse />
        <Textbox value="foo" onChange={() => {}} reverse />
      </>,
    );
    expect(loggerSpy).toHaveBeenCalledWith(
    ...
    const callsWithReverseMessage = loggerSpy.mock.calls.filter((call) =>
      call[0].includes("The `reverse` prop has been deprecated"),
    );
    expect(callsWithReverseMessage).toHaveLength(1);
    loggerSpy.mockRestore();
  });
