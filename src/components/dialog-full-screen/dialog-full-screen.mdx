...
### With overflowing text-only content

When a `DialogFullScreen` has text-only content that exceeds the viewport height, it will scroll. To maintain accessibility, wrap this content in a focusable element. 
This allows keyboard users to navigate to and through the content, ensuring all users can access the information regardless of input method or assistive technology.

<Canvas of={DialogFullScreenStories.FocusableOverflowContentStory} />
