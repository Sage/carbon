...
} from "../../../src/components/simple-color-picker/components.test-pw";
...
test.describe(
  "Should render SimpleColorPicker component and check functionality",
  () => {
    ...
    testData.forEach((name) => {
      test(`should render with name set to ${name}`, async ({
        mount,
        page,
      }) => {
    ...
    ([
      ...
      ([maxWidth, childWidth, bottomLess, topLess, leftLess]) => {
        test(`should render with childWidth prop set to ${maxWidth}`, async ({
          mount,
          page,
        }) => {
          await mount(
    ...
    ([
      ...
      test(`should render with ${type} as a string`, async ({
        mount,
        page,
      }) => {
        await mount(
    ...
    ([
      ...
      test(`should render with ${type} as a boolean`, async ({
        mount,
        page,
      }) => {
        await mount(
...
test.describe(
  "Should render SimpleColorPicker component and check events",
  () => {
    ...
    [
      ...
      test(`should call onChange callback and focus the correct item when the up arrow key is pressed from the element with index ${indexPress}`, async ({
        ...
        await expect(
          simpleColorPickerInput(page, focusedIndex)
        ).toHaveAttribute("value", colors[focusedIndex].color);
        await expect(callbackCount).toBe(1);
      });
    });
...
test.describe(
  "Should render SimpleColor component and check functionality",
  () => {
...
test.describe("Should render SimpleColor component and check events", () => {
  test("should call onChange callback when a click event is triggered", async ({
    mount,
    page,
  }) => {
