...
test.each<[TileProps["highlightVariant"], string]>([
  ...
  [
    "gradient",
    "linear-gradient(to bottom,var(--mode-color-ai-alt-stop-1) 0%,var(--mode-color-ai-alt-stop-2) 40%,var(--mode-color-ai-alt-stop-3) 90%)",
  ],
])(
...
test.each([
  ["default", "var(--global-radius-container-xl)"],
  ...
  (roundness, expectedBorderRoundness) => {
    render(
    ...
    expect(tileElement).toHaveStyleRule(
      "border-radius",
      expectedBorderRoundness,
    );
  },
);

test.each([
  ["curved", "var(--global-radius-container-xl)"],
  ["moderate", "var(--global-radius-container-l)"],
] as const)(
  "renders with the expected border roundness when roundness is %s",
  (roundness, expectedBorderRoundness) => {
    render(
      <Tile roundness={roundness} data-role="tile">
        <TileContent>Child 1</TileContent>
