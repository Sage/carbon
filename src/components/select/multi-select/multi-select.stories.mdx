...
This example shows how you could use a custom onChange handler to limit the amount of selected items to a fixed maximum.

Please note that this is not a recommended best practice and may confuse users - it is better to use validation with a clear error message if these are your requirements.
But it demonstrates what can be done with a custom onChange handler in a controlled component.
...
<Canvas>
  <Story name="pills with long text">
