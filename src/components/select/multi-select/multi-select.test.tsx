...
const ControlledMultiSelect = ({
  ...
  const [value, setValue] = React.useState<
    NonNullable<MultiSelectProps["value"]>
  >([]);
  ...
  return (
    <MultiSelect
      ...
      onChange={(event) => {
        setValue(
          (event.target.value as unknown) as NonNullable<
...
describe("when typing in the input", () => {
  it("pressing Backspace key removes the last selected option if it exists", async () => {
    const user = userEvent.setup();

    render(
      <ControlledMultiSelect label="Colour" value={["amber", "black"]}>
        <Option text="amber" value="amber" />
    ...
    expect(
      screen.queryByText("black", { ignore: "li" })
