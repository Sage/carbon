...
test("keeps transparent as a compatibility alias for the subtle variant", () => {
  render(
...
test("applies the subtle variant to the input", () => {
  render(
    <SimpleSelect label="Colour" onChange={() => {}} variant="subtle" value="">
      <Option text="amber" value="amber" />
    </SimpleSelect>,
  );
...
test("applies each input size when multiple sizes are rendered together", () => {
  const sizes = [
    {
      size: "small",
  ...
  screen.getAllByTestId("input-container").forEach((input, index) => {
    expect(input).toHaveStyleRule("max-height", sizes[index].height);
    expect(input).toHaveStyleRule("padding", sizes[index].padding);
  });
});

test("supports the non-deprecated TextInput props", async () => {
  render(
    <SimpleSelect
      autoFocus
      formattedValue="Formatted amber"
      inputHint="Choose a colour"
      label="Colour"
      maxWidth="400px"
      onChange={() => {}}
      prefix="Tone"
  ...
  expect(screen.getByText("Formatted amber")).toBeVisible();
  expect(screen.getByText("Choose a colour")).toBeVisible();
  expect(screen.getByText("Tone")).toBeVisible();
});

test("accepts deprecated props without applying or forwarding them", async () => {
  ...
  render(
    <SimpleSelect
      ...
      align="right"
      deferTimeout={1}
      fieldHelp="Deprecated field help"
      helpAriaLabel="Deprecated help label"
      iconOnClick={deprecatedCallback}
      iconOnMouseDown={deprecatedCallback}
      iconTabIndex={0}
      inert=""
      ...
      label="Colour"
      labelAlign="right"
      labelHelp="Deprecated label help"
      labelSpacing={2}
      labelWidth={50}
      listPlacement="top"
      listWidth={600}
      multiColumn
      onChange={() => {}}
      onChangeDeferred={deprecatedCallback}
      placeholder="Deprecated placeholder"
      reverse
      tableHeader={<span>Deprecated table header</span>}
      tooltipId="deprecated-tooltip"
  ...
  const input = screen.getByRole("combobox");
  expect(input).not.toHaveAttribute("align");
  expect(input).not.toHaveAttribute("inert");
  expect(input).not.toHaveAttribute("placeholder");
  expect(input).not.toBeRequired();
  expect(screen.queryByText("Deprecated field help")).not.toBeInTheDocument();
  expect(screen.queryByText("Deprecated label help")).not.toBeInTheDocument();
  expect(screen.queryByText("Deprecated table header")).not.toBeInTheDocument();

  await user.click(screen.getByTestId("input-icon-toggle"));
  await user.keyboard("a");

  expect(deprecatedCallback).not.toHaveBeenCalled();
...
test("positions the dropdown at bottom by default", async () => {
  const user = userEvent.setup();
  render(
    <SimpleSelect label="Colour" onChange={() => {}} value="amber">
      <Option text="amber" value="amber" />
...
test("calls onListScrollBottom when the open list reaches the bottom", async () => {
  const user = userEvent.setup();
  ...
  [CHARACTERS.DIACRITICS, CHARACTERS.SPECIALCHARACTERS].forEach(
    (labelValue) => {
      test(`renders label with special characters: ${labelValue}`, () => {
        render(
          <SimpleSelect label={labelValue} onChange={() => {}} value="">
            <Option text="Amber" value="amber" />
          </SimpleSelect>,
        );
