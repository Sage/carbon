...
When the list is open, typing printable characters highlights the first option whose text starts with the typed sequence. The
`typeaheadTimeout` prop controls how long the component waits between characters before starting a new sequence, in milliseconds.
It defaults to `1500`.
