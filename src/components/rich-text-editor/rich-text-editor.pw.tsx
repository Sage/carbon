...
test.describe("Prop tests", () => {
  test.describe("characterLimit", () => {
    test(`value of 0`, async ({ mount, page }) => {
      await mount(<RichTextEditorDefaultComponent characterLimit={0} />);
    ...
    [100, 500, 3000].forEach((characterLimit) => {
      test(`value of ${characterLimit}`, async ({ mount, page }) => {
        await mount(
          <RichTextEditorDefaultComponent characterLimit={characterLimit} />,
        );
        let displayedLimit = await page
          .locator("div[data-role='pw-rte-character-limit']")
          .textContent();
        expect(displayedLimit).toBe(`${characterLimit} characters remaining`);
  ...
  test.describe("inputHint", () => {
    test(`value of 'hint'`, async ({ mount, page }) => {
      await mount(<RichTextEditorDefaultComponent inputHint="hint" />);
    ...
    test(`value not provided`, async ({ mount, page }) => {
      await mount(<RichTextEditorDefaultComponent />);
      const hint = await page
        .locator(`div[data-role='pw-rte-input-hint']`)
        .count();
      expect(hint).toBe(0);
  ...
  test.describe("isOptional", () => {
    [true, false].forEach((isOptional) => {
      test(`value of ${isOptional}`, async ({ mount, page }) => {
        await mount(<RichTextEditorDefaultComponent isOptional={isOptional} />);
  ...
  test.describe("labelText", () => {
    [{ value: "Rich Text Editor" }].forEach(({ value }) => {
      test(`value of ${value}`, async ({ mount, page }) => {
        await mount(<RichTextEditorDefaultComponent labelText={value} />);
  ...
  test.describe("placeholder", () => {
    [undefined, "Enter text here"].forEach((placeholder) => {
      test(`value of ${placeholder}`, async ({ mount, page }) => {
        await mount(
          <RichTextEditorDefaultComponent placeholder={placeholder} />,
        );
  ...
  test.describe("previews", () => {
    test("simple", async ({ mount, page }) => {
      ...
      await mount(<RichTextEditorDefaultComponent previews={previews} />);
  ...
  test.describe("required", () => {
    [true, false].forEach((required) => {
      test(`value of ${required}`, async ({ mount, page }) => {
        await mount(<RichTextEditorDefaultComponent required={required} />);
        const content = await page.evaluate(
...
test.describe("Functionality tests", () => {
  test.describe("onCancel", () => {
    test("resets the content of the editor when resetOnCancel is set to true", async ({
      mount,
      page,
    }) => {
      await mount(
...
test.describe("Styling tests", () => {
  test.describe("error", () => {
    test(`value of 'Error message'`, async ({ mount, page }) => {
      await mount(<RichTextEditorDefaultComponent error="Error message" />);
  ...
  test.describe("rows", () => {
    [
      ...
      test(`has the correct height with a rows property of ${rows}`, async ({
        ...
        await mount(<RichTextEditorDefaultComponent rows={rows} />);
        await expect(
  ...
  test.describe("warning", () => {
    test(`value of 'Warning message'`, async ({ mount, page }) => {
      await mount(<RichTextEditorDefaultComponent warning="Warning message" />);
      const displayedWarning = await page
        .locator("div[data-role='pw-rte-validation-message']")
        .textContent();
...
test.describe("Accessibility tests", () => {
  ...
  test(`should pass for default component in error state`, async ({
    ...
    await mount(<RichTextEditorDefaultComponent error="This is an error" />);
  ...
  test(`should pass for default component in warning state`, async ({
    ...
    await mount(<RichTextEditorDefaultComponent warning="This is a warning" />);
