...
Cards can be made draggable by using the `draggable` prop.

The example uses the `useDraggable` hook. Pass a valid node or array of nodes to the `useDraggable` hook. 
This makes the node(s) draggable and returns them in a container.

The `DraggableProvider` component wraps these containers. Use it when dragging between containers is required.
```javascript
import useDraggable, { DraggableProvider } from "carbon-react/lib/hooks/useDraggable";
```
