      ...
      <Box width="700px" height="450px">
        ...
        >
          <DndProvider backend={Backend}>
            <Column title={productOne} dataElement="draggable-container-1">{returnItemsForColumn(productOne)}</Column>
            <Column title={productTwo} dataElement="draggable-container-2">{returnItemsForColumn(productTwo)}</Column>
          </DndProvider>
        </div>
      </Box>
    );
