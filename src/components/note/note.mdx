...
Inline controls can also be displayed when the `Note` component does not have a title.

<Canvas of={NoteStories.WithInlineControlsWithoutTitle} />

### With status

An optional status can be provided using the `status` prop. It is displayed with
its timestamp below the author and created details.

<Canvas of={NoteStories.WithStatus} />

### With @mentions

Mentions saved by the Text Editor are rendered as read-only pills in a Note.

<Canvas of={NoteStories.WithMention} />
