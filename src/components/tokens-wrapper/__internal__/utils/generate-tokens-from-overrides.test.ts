...
test("overrideTokens strips existing alpha channel from hex color before applying opacity", () => {
  const overrides = {
    light: {
      primaryBrand: "#ff000080", // red with alpha already
    },
  };
  const result = overrideTokens(overrides);

  // The base color should be used as-is (alpha preserved for the default token)
  expect(result).toContain("--mode-color-action-main-default: #ff000080;");
  // The opacity variants should use the 6-digit base (#ff0000) with new alpha appended,
  // not produce an invalid 10-digit hex (#ff000080cc)
  expect(result).toContain("--mode-color-action-main-default-alt: #ff0000cc;");
  expect(result).toContain("--mode-color-action-main-default-alt3: #ff000008;");
});

test("overrideTokens strips existing alpha channel from 4-digit hex before applying opacity", () => {
  const overrides = {
    light: {
      primaryBrand: "#f008", // shorthand red with alpha
    },
  };
  const result = overrideTokens(overrides);

  expect(result).toContain("--mode-color-action-main-default: #f008;");
  expect(result).toContain("--mode-color-action-main-default-alt: #ff0000cc;");
  expect(result).toContain("--mode-color-action-main-default-alt3: #ff000008;");
});
