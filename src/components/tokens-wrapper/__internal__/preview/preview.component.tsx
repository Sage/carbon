...
const Preview: React.FC<PreviewProps> = ({
  ...
  const [borderRadiusScaleInput, setBorderRadiusScaleInput] = useState(
    String(initialBorderRadiusScale),
  );
  ...
  const resetColors = () => {
    ...
    setBorderRadiusScaleInput(String(initialBorderRadiusScale));
  };
      ...
      <Container>
        ...
        {isWhiteLabel && !isChromatic && (
          <Sidebar $isDarkMode={isDarkMode}>
            ...
            {renderColorInput(
              "Focus Primary Inverse",
              "ring",
              focusInverseOverrides.ring || "",
              setFocusInverseOverrides,
            )}
            {renderColorInput(
              "Focus Secondary Inverse",
              "contrast",
              focusInverseOverrides.contrast || "",
              setFocusInverseOverrides,
            )}
            {renderColorInput(
              "Focus Alt Inverse",
            ...
            <div style={{ marginBottom: "16px" }}>
              <Input
                ...
                type="text"
                value={borderRadiusScaleInput}
                onChange={(e) => {
                  const raw = e.target.value;
                  setBorderRadiusScaleInput(raw);

                  if (raw === "" || raw === ".") {
                    setBorderRadiusScale(undefined);
                    return;
                  }

                  const parsed = Number(raw);
                  if (!isNaN(parsed)) {
                    setBorderRadiusScale(parsed);
                  }
                }}
                placeholder="0.5"
              />
