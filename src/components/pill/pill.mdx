...
When a callback in the `onDelete` prop is provided, a remove button will be rendered and clicking on it will trigger that callback. Use the `ariaLabelOfRemoveButton` prop to give the remove button a clear, unique accessible label that describes its relationship to the pill.

<Canvas of={PillStories.WithRemoveButton} />

#### Inverse

Use the `inverse` prop to apply inverse styling to the pill.
