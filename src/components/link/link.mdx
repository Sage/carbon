...
Setting the `variant` prop to either `negative`, `neutral` or `white` enables overriding of the `default` styling of the `Link` component.

> **Note**: The `white` variant is designed to be used on dark backgrounds, and is only applied when the `isDarkBackground` prop is `“true”`.
