...
Setting the `variant` prop to either `negative`, or `subtle` enables overriding of the `typical` variant which is the default styling of the `Link` component.
...
When the `Link` component is rendered on a dark background, the `inverse` prop can be used to update the colour styling
