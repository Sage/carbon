...
**Please note**: To disable an anchor, the `href` prop must be not defined.
...
<Canvas>
  <Story name="with icon" parameters={{ chromatic: { disable: true } }}>
    <Link icon="settings" href="#foo">This is a link</Link>
  </Story>
</Canvas>
...
<Canvas>
  <Story name="with tooltip" parameters={{ chromatic: { disable: true } }}>
    <div style={{ margin: "64px" }}>
      <Link icon="settings" tooltipMessage="This is a tooltip message" href="#foo">
        This is a link
      </Link>
    </div>
  </Story>
...
<Canvas>
  <Story name="with isSkipLink" parameters={{ chromatic: { disable: true } }}>
