...
For legacy support, the `Button` component can also accept the following props:
- the `buttonType` prop, which maps to the `variant` and `variantType` props as follows:
  ...
  - `subtle` -> `variant="default"` and `variantType="subtle"`
  - `destructive` -> `variant="destructive"` and `variantType="primary"`
  - `destructive-secondary` -> `variant="destructive"` and `variantType="secondary"`
