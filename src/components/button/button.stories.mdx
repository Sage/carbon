...
### Bottom margin
The `mb` prop controls the bottom margin of the button. It is an integer which is multiplied by the base theme constant (8px). 

<Preview>
  <Story name="all types/bottom-margin" parameters={{ info: { disable: true }}} >
  <>
    <div>
      <Button mb={1}>8px bottom margin</Button>
    </div>
    <div>
      <Button mb={2}>16px bottom margin</Button>
    </div>
    <div>
      <Button mb={3}>24px bottom margin</Button>
    </div>
  </>
  </Story>
</Preview>
