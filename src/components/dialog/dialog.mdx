...
<DeprecationWarning>
  This version of the Dialog component is deprecated and will be removed in a future release.
  The legacy interface has been adapted to use the new implementation, but some
  features could not be maintained and have been removed. Please migrate to the{" "}
  <a style={{ color: "white" }} href="../?path=/docs/dialog--docs">
    {" "}
...
Setting the `highlightVariant` prop will add a highlight above the Dialog. Please note that this prop does not work when the `fullscreen` prop is set to `true`.
