...
```javascript
import Dialog from "carbon-react/lib/components/__internal__/dialog/__next__";
```
...
### With gradientKeyLine

Setting the `gradientKeyLine` prop will add a gradient-styled keyline to the dialog header.
...
```javascript
import Dialog, { withDialogHeader } from "carbon-react/lib/components/__internal__/dialog/__next__/dialog.component";
