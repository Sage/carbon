...
context("Testing Dialog component", () => {
  describe("Should render dialog component", () => {
    it.each([[0], [1], [100], [1000]])(
      "should render Dialog component with % as a height parameter",
      (height) => {
    ...
    it.each(specialCharacters)("should check Dialog % title", (title) => {
      CypressMountWithProviders(<DialogComponent title={title} />);

      dialogTitle().should("have.text", title);
    });

    it.each(specialCharacters)("should check Dialog % subtitle", (subtitle) => {
      CypressMountWithProviders(<DialogComponent subtitle={subtitle} />);
    ...
    it("should render Dialog with enabledEscKey prop and should be closed after clicking Escape button", () => {
      CypressMountWithProviders(<DialogComponent focusFirstElement />);
      dialogPreview().should("exist");
    ...
    it.each([["top"], ["topRight"], ["right"]])(
      "Click on background outside Dialog at %s position, Dialog should remain open",
      (position) => {
