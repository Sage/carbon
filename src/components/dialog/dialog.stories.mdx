...
<Preview>
  <Story name="focusing a different first element" parameters={{ chromatic: { disable: true } }}>
    {() => {
      ...
      return (
        <>
          <Button onClick={() => setIsOpenOne(true)}>Open Demo using focusFirstElement</Button>
          <Dialog
          ...
          <Button ml={2} onClick={() => setIsOpenTwo(true)}>Open Demo using autoFocus</Button>
          <Dialog
            disableAutoFocus
            open={isOpenTwo}
