  ...
  return (
    <ModalContext.Provider value={{ isAnimationComplete, triggerRefocusFlag }}>
      <FocusTrap
        ...
        {...rest}
      >
        <div aria-modal role="dialog" ref={ref} tabIndex={tabIndex}>
          {children}
          {shouldFocusFirstElement && (
...
describe("FocusTrap", () => {
  ...
  it("does not focus the first focusable element by default when autoFocus is false", () => {
    render(mockComponentToRender({ autoFocus: false }));
    expect(screen.getByRole("dialog")).not.toHaveFocus();
  });

  it("does not focus the first focusable element by default when isAnimationComplete is false", () => {
  ...
  it("should not throw error if wrapper ref isn't found", () => {
  ...
  it("only focuses elements which meet the custom selector, when tabbing both forwards and backwards and custom focusable element selector is provided", async () => {
    const user = userEvent.setup({ delay: null });
    render(
      <MockComponent focusableSelectors="button.focusable">
        <button type="button" className="focusable">
  ...
  describe("preventDefault", () => {
    it("when focusableSelectors is not used, preventDefault is not called upon tab press", async () => {
      render(
        <MockComponent>
          <button type="button">One</button>
          <button type="button">Two</button>
    ...
    it("when focusableSelectors is used, preventDefault is called when needed to prevent an undesired element becomed focused", async () => {
      render(
        <MockComponent focusableSelectors="button.focusable">
          <button type="button">One</button>
          <button type="button" className="focusable">
