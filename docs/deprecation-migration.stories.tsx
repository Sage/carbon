...
import VerticalDivider from "../src/components/vertical-divider";
...
export const Number: StoryObj = () => {
  ...
  const hasError = state !== "" && !isValidNumber(state);
...
export const DismissibleBox: StoryObj = () => {
  return (
    ...
    >
      ...
      <VerticalDivider p="var(--spacing000)" px="var(--spacing110)" />
      <Image
...
export const BasicGrid: StoryObj = () => (
  <Box
    display="grid"
    gridTemplateColumns="repeat(12, 1rf)"
    gridAutoRows="auto"
...
export const ResponsiveGrid: StoryObj = () => {
  ...
  const getItemStyles = (itemNumber: number) => {
    const baseStyles = {
      alignSelf: "stretch",
      justifySelf: "stretch",
    };
    ...
    if (isSmall) {
      return {
        ...
        maxWidth: "900px",
        gridColumn: "1 / 13",
        gridRow: `${itemNumber} / ${itemNumber}`,
      };
    }
    const breakpoints = {
      medium: { maxWidth: "1300px", active: isMedium },
      large: { maxWidth: "1500px", active: isLarge },
    };

    const activeBreakpoint = Object.values(breakpoints).find((bp) => bp.active);

    if (activeBreakpoint) {
      const layouts: Record<number, { gridColumn: string; gridRow: string }> = {
        1: { gridColumn: "1 / 7", gridRow: "1 / 1" },
        2: { gridColumn: "7 / 13", gridRow: "1 / 1" },
        3: { gridColumn: "1 / 13", gridRow: "2 / 2" },
      };
    ...
    return {
      ...baseStyles,
      gridColumn: `${(itemNumber - 1) * 4 + 1} / ${itemNumber * 4 + 1}`,
      gridRow: "1 / 1",
    };
  };
  ...
  return (
    ...
    >
      {[1, 2, 3].map((itemNumber) => (
        <Box key={itemNumber} {...getItemStyles(itemNumber)}>
          <Tile>{itemNumber}</Tile>
        </Box>
      ))}
    </Box>
  );
...
export const InlineInputs: StoryObj = () => {
  ...
  return (
    <>
      <Box display="flex" flexDirection="row" alignItems="center" width="100%">
        <Typography
          id="inline-inputs-default"
          as="label"
          m={0}
          pr="var(--spacing200)"
          fontWeight="var(--fontWeights500)"
        >
          Input components made inline
        </Typography>

        <Box flex="1">
          <Textbox aria-labelledby="inline-inputs-default" />
        </Box>
...
InlineInputs.storyName = "Inline Inputs";
