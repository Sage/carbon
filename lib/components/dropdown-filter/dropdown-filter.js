      ...
      if (!_this.blockBlur) {
        ...
        if (_this.props.freetext) {
          ...
          if (_this.state.filter) {
            opt = _this.props.options.find(function (opt) {
              if (opt.get('name')) {
                return opt.get('name').toLowerCase() === _this.state.filter.toLowerCase();
              }
            });
          }
...
DropdownFilter.propTypes = {
   ...
   * @type {Boolean}
   */
  freetext: _react2.default.PropTypes.bool,
   ...
   * @type {String}
   */
  freetextName: _react2.default.PropTypes.string
};
