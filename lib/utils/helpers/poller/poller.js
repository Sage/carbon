    ...
    _superagent2.default.get(queryOpts.url).query(queryOpts.data).set(queryOpts.headers).end(function (err, response) {
      ...
      if (err) {
        if (funcs.handleError) {
          ...
          result = _logger2.default.error(err.message); // eslint-disable-line no-console
        }
...
function getFunctions(functions) {
  return {
    ...
    onMaxRetries: functions.onMaxRetries || function () {
      _logger2.default.warn('The poller has made too many requests - terminating poll'); // eslint-disable-line no-console
    }
  };
}
...
function setupValid(queryOptions, functions) {
  if (queryOptions === null || typeof queryOptions.url === 'undefined') {
    _logger2.default.error('You must provide a url to the poller'); // eslint-disable-line no-console
    return false;
  }
  ...
  if (typeof functions.conditionMet !== 'undefined' && typeof functions.callback === 'undefined') {
    ...
    _logger2.default.error(msg); // eslint-disable-line no-console
    return false;
  }
  return true;
}