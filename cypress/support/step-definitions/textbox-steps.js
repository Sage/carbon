...
Then("Textbox multiple component is not readOnly", () => {
  ...
  textbox(positionOfElement("first"))
    .should("not.have.css", "background-color", borderNoReadonlyColor)
...
Then("Multiple Textbox component is labelInline", () => {
  getDataElementByValueAndPosition("label", positionOfElement("first"))
    .parent()
    .should("have.css", TEXT_ALIGN, TEXT_ALIGN_END);
  getDataElementByValueAndPosition("label", positionOfElement("second"))
    .parent()
    .should("have.css", TEXT_ALIGN, TEXT_ALIGN_END);
});
...
Then("Textbox component is not labelInline", () => {
  getDataElementByValue("label")
    .parent()
    .should("not.have.css", TEXT_ALIGN, TEXT_ALIGN_START);
});

Then("Multiple Textbox component is not labelInline", () => {
  labelByPosition(positionOfElement("first")).should(
    "not.have.css",
    TEXT_ALIGN,
    "left"
  );
  labelByPosition(positionOfElement("second")).should(
    "not.have.css",
    TEXT_ALIGN,
    "left"
  );
...
Then("Multiple Textbox inputWidth is set to {string}", (width) => {
  textboxByPosition(positionOfElement("first"))
    .parent()
    .should("have.css", "flex", `0 0 ${width}%`);
  textboxByPosition(positionOfElement("second"))
    .parent()
    .should("have.css", "flex", `0 0 ${width}%`);
});
...
Then("Multiple label Align on preview is {string}", (direction) => {
  if (direction === "left") {
    labelByPosition(positionOfElement("first"))
      .parent()
      .should(($element) =>
        expect($element).to.have.css(TEXT_ALIGN, TEXT_ALIGN_START)
      );
    labelByPosition(positionOfElement("second"))
      .parent()
      .should(($element) =>
        expect($element).to.have.css(TEXT_ALIGN, TEXT_ALIGN_START)
      );
