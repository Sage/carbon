...
const THEME_PREFIX = 'carbon-table carbon-table--';
...
When('row number {int} is clickable', (rowNumber) => {
  ...
  rowByNumber(rowNumber).should('have.class', 'carbon-table-row--clickable');
});

When('row number {int} is not clickable', (rowNumber) => {
  cy.wait(500); // required because we wait for class change inside iFrame
  rowByNumber(rowNumber).should('not.have.class', 'carbon-table-row--clickable');
});

When('row number {int} is highlighted', (rowNumber) => {
  rowByNumber(rowNumber).should('have.class', 'carbon-table-row--highlighted');
});

When('row number {int} is not highlighted', (rowNumber) => {
  rowByNumber(rowNumber).should('not.have.class', 'carbon-table-row--highlighted');
});
