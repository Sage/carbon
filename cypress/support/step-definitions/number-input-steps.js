...
Then("{word} input component is not {word}", (parameter) => {
  if (parameter === "disabled") {
    ...
    commonDataElementInputPreviewNoIframe()
      .parent()
      .should("not.have.attr", parameter);
  }
});

Then(
  "{word} input component size is set to {string} and has min-height set to {int} and paddings set to {int}",
  (componentName, size, minHeight, px) => {
    commonDataElementInputPreviewNoIframe()
      .parent()
      .should("have.css", "min-height", `${minHeight}px`)
