...
Then(
  "I type numeral characters {string} in {string} inputs",
  (text, position) => {
    numeralDateInputByPosition(positionOfElement(position)).clear().type(text);
    numeralDateInputByPosition(positionOfElement(position))
      .parent()
...
When(
  "I type numeral characters {string} in {string} inputs and blur",
  (text, position) => {
    numeralDateInputByPosition(positionOfElement(position))
      .clear()
      .type(text)
...
Then("{string} numeral input is set to {string}", (position, text) => {
  numeralDateInputByPosition(positionOfElement(position)).should(
    "have.value",
    text
  );
});
...
Then("inputs have value {string}", (text) => {
  numeralDateInputByPosition(positionOfElement("first")).should(
    "have.value",
    text
  );
  numeralDateInputByPosition(positionOfElement("second")).should(
    "have.value",
    text
  );
  numeralDateInputByPosition(positionOfElement("third")).should(
    "have.value",
    text
  );
});
