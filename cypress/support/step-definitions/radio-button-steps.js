...
Then('{string} RadioButton has value {word}', (position, text) => {
  radioButtonInputByPosition(positionOfElement(position)).should('have.attr', 'value').should('contain', text);
});
...
Then('{string} RadioButton component is enabled', (position) => {
  labelByPosition(positionOfElement(position)).should('not.be.disabled')
    .and('not.have.attr', 'disabled');
});

Then('{string} RadioButton is set to reverse', (position) => {
  radioButtonComponentByPosition(positionOfElement(position)).find('div:nth-child(2) input').should('have.attr', 'role', 'radio');
});

Then('{string} RadioButton is not set to reverse', (position) => {
  radioButtonInputByPosition(positionOfElement(position)).should('have.attr', 'role', 'radio');
});

Then('{string} RadioButton size on preview is set to {string}', (position, size) => {
  if (size === 'small') {
    radioButtonInputByPosition(positionOfElement(position)).should('have.css', 'width', '16px')
      .and('have.css', 'height', '16px');
  } else {
    radioButtonInputByPosition(positionOfElement(position)).should('have.css', 'width', '24px')
