...
Then('{string} RadioButton has value {string}', (radioButtonName, text) => {
  cy.radioButton(radioButtonName, 'div:nth-child(1) input').should('have.attr', 'value').should('contain', text);
});

Then('{string} RadioButton component is disabled', (radioButtonName) => {
  cy.radioButton(radioButtonName, 'div:nth-child(1) input').should('have.attr', 'type', 'radio')
    .and('be.disabled')
...
Then('{string} RadioButton is not set to reverse', (radioButtonName) => {
  cy.radioButton(radioButtonName, 'div:nth-child(1) input').should('have.attr', 'role', 'radio');
});

Then('{string} RadioButton size on preview is set to {string}', (radioButtonName, size) => {
  if (size === 'small') {
    cy.radioButton(radioButtonName, 'div:nth-child(1) input').should('have.css', 'width', '16px')
      .and('have.css', 'height', '16px');
  } else {
    cy.radioButton(radioButtonName, 'div:nth-child(1) input').should('have.css', 'width', '24px')
