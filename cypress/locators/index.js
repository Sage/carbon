...
export const getKnobsInputByGroup = (groupName, propertyName) =>
  cy.get(`[name="${propertyName}_${groupName}"]`);
...
export const getDataElementByValueAndPosition = (element, position) =>
  cy.get(`[data-element="${element}"]`).eq(position);
export const commonDataElementInputPreviewNoIframe = () =>
  cy.get(COMMMON_DATA_ELEMENT_INPUT);
export const commonDataElementInputPreviewByPositionNoIFrame = (position) =>
  cy.get(COMMMON_DATA_ELEMENT_INPUT).eq(position);
