Feature: Pager component
  ...
  Scenario Outline: Set totalRecords to <totalRecords>
    When I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "<nameOfObject>" object name
  ...
  Scenario: Set totalRecords to 1 and check spell of item word
    When I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "totalRecords1" object name
  ...
  Scenario: Set pageSize to 1 item
    When I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "pageSize1" object name
    Then pageSize is set to "1" item
      And I am on 1st of "100" pages

  @positive
  Scenario: Enable showPageSizeSelection and verify default value
    When I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "showPageSizeSelection" object name
    Then pageSize is visible
      And pageSize is set to "10" items

  @positive
  Scenario: Disable showPageSizeSelection
    When I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "showPageSizeSelectionFalse" object name
    Then pageSize is not visible

  @positive
  Scenario Outline: Pagination <button> button is disabled after clicking on last button
    Given I open Test test_default "Pager" component in noIFrame with "pager" json from "test" using "default" object name
