...
A basic `Select` with `Option` children. The selected option's `text` value is displayed in the input field.
...
Set `openOnFocus` to open the option list automatically when the input receives focus.
...
Set `readOnly` to display the current value without allowing changes. The dropdown cannot be opened.

See: `examples/Readonly.md`

### Transparent

Set `transparent` to render the select without a visible border or background, useful when embedding it inside other styled containers.

See: `examples/Transparent.md`

### Transparent Disabled

Combines the `transparent` and `disabled` props to render a borderless, non-interactive select.
...
Use `OptionGroupHeader` to group related options under a labelled section heading within the dropdown list.

See: `examples/OptionGroups.md`
...
See: `examples/OptionGroupsWithComposedChildren.md`
...
Combines multi-column layout with `enableVirtualScroll` for performant rendering of large tabular option lists.
