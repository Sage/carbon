...
Render a single icon by passing its `type` prop (e.g. `type="add"`). The `type` prop is required on every `Icon`.
...
| disabled | boolean \| undefined | No |  | Yes | Applies disabled styling to the icon. |  |  |
