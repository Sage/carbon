...
Component that allows to choose multiple options from the drop-down list.
...
A basic `MultiSelect` with `Option` children. Selected options appear as pills inside the input. Type to filter the list.
...
Set `disabled` to prevent user interaction. The input and dropdown are visually dimmed and non-interactive, and existing pills cannot be removed.

See: `examples/Disabled.md`

### Read Only

Set `readOnly` to display the current selection as pills without allowing changes. The filter input is disabled but the value remains visible and accessible.
...
See Colors for more information on usable colors

See: `examples/WithCustomColoredPills.md`
...
See: `examples/PillsWithLongText.md`
